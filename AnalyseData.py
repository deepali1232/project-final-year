...
class Analyse:
    ...
    def __init__(self, path):
        ...
        if path == r'datasets/country.csv':
            ...
            self.df["Total_vaccinations(count)_per_hundred"] = self.df.groupby("country").total_vaccinations_per_hundred.tail(1)
            self.df["People_vaccinated(count)_per_hundred"] = self.df.groupby("country").people_vaccinated_per_hundred.tail(1)
            self.df["People_fully_vaccinated(count)_per_hundred"] = self.df.groupby("country").people_fully_vaccinated_per_hundred.tail(1)
        # self.cleanData()
